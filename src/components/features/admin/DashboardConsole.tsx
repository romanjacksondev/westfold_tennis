...
import TournamentTypeVisual from '@/components/features/admin/TournamentTypeVisual';
import TournamentPreparationModal from '@/components/features/admin/TournamentPreparationModal';
...
type Options = { players: RecordItem[]; venues: RecordItem[]; surfaces: RecordItem[]; categories: RecordItem[]; types: RecordItem[]; tournaments: RecordItem[]; users: RecordItem[] };
...
const simpleFields: Record<Exclude<Resource, 'tournaments' | 'matches'>, string[]> = {
  players: ['name', 'nickname', 'lastname', 'mail', 'phone', 'userId'],
...
const labels: Record<string, string> = { name: 'Nombre', nickname: 'Apodo', lastname: 'Apellido', mail: 'Email de contacto', email: 'Email de acceso', phone: 'Teléfono', address: 'Dirección', date: 'Fecha', venueId: 'Sede', surfaceId: 'Superficie', championId: 'Campeón', tournamentCategoryId: 'Categoría', tournamentTypeId: 'Tipo', tournamentId: 'Torneo', player1Id: 'Jugador 1', player2Id: 'Jugador 2', winnerId: 'Ganador', role: 'Rol', password: 'Contraseña', status: 'Estado', drawSize: 'Tamaño del cuadro', qualifiers: 'Clasificados a playoffs', userId: 'Cuenta de usuario', playerId: 'Perfil de jugador' };
const tournamentStatusLabels: Record<string, string> = { PREPARATION: 'En preparación', IN_PROGRESS: 'En curso', FINISHED: 'Terminado', CANCELLED: 'Cancelado' };
...
export default function DashboardConsole() {
  ...
  const [options, setOptions] = useState<Options>({ players: [], venues: [], surfaces: [], categories: [], types: [], tournaments: [], users: [] });
  const [showTournamentTypeInfo, setShowTournamentTypeInfo] = useState(false);
  const [prepTournament, setPrepTournament] = useState<RecordItem | null>(null);
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await requestJson(`/api/admin/${resource}`);
      setItems(data);
      if (resource === 'users' || resource === 'players') {
        const [players, users] = await Promise.all([
          requestJson('/api/admin/players'),
          requestJson('/api/admin/users'),
        ]);
        setOptions((prev) => ({ ...prev, players, users }));
      }
    }
  ...
  async function autoUnify() {
    if (!window.confirm('¿Deseas vincular automáticamente jugadores y usuarios que compartan el mismo email?')) return;
    setUnifying(true);
    setMessage(null);
    try {
      const res = await requestJson('/api/admin/unify-users-players', { method: 'POST' });
      setMessage({ type: 'success', text: res.message });
      await load();
    } catch (error) {
      setMessage({ type: 'error', text: error instanceof Error ? error.message : 'Error en la auto-unificación' });
    } finally {
      setUnifying(false);
    }
  }
  ...
  useEffect(() => {
    ...
    if (resource === 'tournaments' || resource === 'matches' || resource === 'users' || resource === 'players') {
      Promise.all([
        ...
        requestJson('/api/admin/users'),
      ]).then(([players, venues, surfaces, categories, types, tournaments, users]) => {
        setOptions({ players, venues, surfaces, categories, types, tournaments, users });
  ...
  const requiredFields = resource === 'users' ? ['email', 'role', ...(!editing ? ['password'] : [])] : fields.filter((field) => field !== 'date' && field !== 'championId' && field !== 'drawSize' && field !== 'qualifiers' && field !== 'userId' && field !== 'playerId' && field !== 'nickname' && field !== 'lastname' && field !== 'mail' && field !== 'phone');
  const selectedTournamentType = options.types.find((option) => option.id === form.tournamentTypeId);

  function startEdit(item: RecordItem) {
    if (resource === 'tournaments' && item.status === 'FINISHED') {
      return;
    }
    if (resource === 'tournaments' && item.status === 'PREPARATION') {
      setPrepTournament(item);
      return;
    }
    setEditing(item);
    ...
    for (const field of fields) {
      if (field === 'userId') {
        initialForm[field] = (item.user as { id?: string } | undefined)?.id ?? (item.userId as string) ?? '';
      } else if (field === 'playerId') {
        initialForm[field] = (item.player as { id?: string } | undefined)?.id ?? (item.playerId as string) ?? '';
      } else {
        initialForm[field] = String(item[field] ?? '');
      }
    }
    setForm(initialForm);
    setMessage(null);
  ...
  function resetForm() { setEditing(null); setPrepTournament(null); setForm(resource === 'tournaments' ? newTournamentForm() : { name: '', role: 'USER' }); setMatchSets([emptyMatchSet()]); setSelectedPlayerIds([]); }
