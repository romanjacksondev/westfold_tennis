...
export default function DashboardConsole({ userName }: { userName: string }) {
  ...
  useEffect(() => {
    setEditing(null); setForm({ name: '', role: 'USER' }); setMessage(null); load();
    if (resource === 'tournaments' || resource === 'matches' || resource === 'users') {
      ...
      ]).then(([players, venues, surfaces, categories, types, tournaments]) => setOptions({ players, venues, surfaces, categories, types, tournaments })).catch(() => setMessage({ type: 'error', text: 'No se pudieron cargar las opciones relacionadas.' }));
    }
  }, [resource]);
  ...
  const fields = isSimple ? simpleFields[resource as Exclude<Resource, 'tournaments' | 'matches'>] : resource === 'tournaments' ? ['name', 'date', 'venueId', 'surfaceId', 'championId', 'tournamentCategoryId', 'tournamentTypeId'] : ['tournamentId', 'player1Id', 'player2Id', 'winnerId'];
  ...
  function resetForm() { setEditing(null); setForm({ name: '', role: 'USER' }); }
  ...
  return (
    <main className="admin-shell">
      <aside className="admin-sidebar">
        <div className="brand-mark"><span>WT</span><div><strong>Westfold</strong><small>Admin console</small></div></div>
