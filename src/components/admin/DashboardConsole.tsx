...
export default function DashboardConsole({ userName }: { userName: string }) {
  ...
  const requiredFields = resource === 'users' ? ['email', 'role', ...(!editing ? ['password'] : [])] : fields.filter((field) => field !== 'date' && field !== 'championId');
